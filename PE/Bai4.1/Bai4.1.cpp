...
void printAsciiString(const std::string& text)
{
...
std::size_t fileSize,
std::size_t& fileOffset,
std::size_t* matchedSectionIndex = nullptr)
{
        ...
        ++i)
    {
        ...
        const std::uint64_t calculatedOffset =
...
std::wstring getSectionPermissions(
    ...
    std::size_t sectionTableOffset)
{
         ...
         ++i)
    {
        ...
        const std::size_t currentHeaderOffset =
            sectionTableOffset +
    ...
    const std::vector<IMAGE_SECTION_HEADER>& sections)
{
     ...
     */
    const std::uint64_t functionsTableSize64 =
        static_cast<std::uint64_t>(
            exportDirectory.NumberOfFunctions
            ) * sizeof(DWORD);

    const std::uint64_t namesTableSize64 =
        static_cast<std::uint64_t>(
            exportDirectory.NumberOfNames
            ) * sizeof(DWORD);

    const std::uint64_t ordinalsTableSize64 =
        static_cast<std::uint64_t>(
            exportDirectory.NumberOfNames
            ) * sizeof(WORD);
        ...
        ++i)
    {
        ...
        std::size_t functionNameOffset = 0;
         ...
         */
        const std::uint64_t exportStart =
            exportData.VirtualAddress;
...
void printImportDirectory(
    ...
    const vector<IMAGE_SECTION_HEADER>& sections)
{
    wcout << L"\n[DEBUG] Đã gọi printImportDirectory()\n";
...
int wmain(int argc, wchar_t* argv[])
{
    ...
    wcout << L"\n[DEBUG] Đã hoàn thành Section Headers\n";
